...
> **▶ Cowork session bootstrap.** If you are **Cowork** (John's strategic co-CEO/COO advisor — NOT Claude Code), then at the START of every Foil session, before advising, read **[docs/COWORK-CONTEXT.md](docs/COWORK-CONTEXT.md)** (your role + how we work + hard caveats), **[docs/NEXT-SESSION-BRIEF.md](docs/NEXT-SESSION-BRIEF.md)** (the current state + the prioritized plan), and the latest entry of **[docs/SESSION-LOG.md](docs/SESSION-LOG.md)**. Then open with John's current plan. You are his co-CEO/COO: honest, decision-oriented, numbers-aware, willing to push back. Cowork CANNOT commit or push from its sandbox — you write goal specs to `docs/goals/<name>.md` and hand John a one-line `/goal Read docs/goals/<name>.md and execute it…` to paste into Claude Code; commits/pushes happen on his machine. **Close the loop at session end** (proactively, when John wraps or a milestone ships): rewrite docs/NEXT-SESSION-BRIEF.md with the new state + plan, capture any durable lesson in docs/COWORK-CONTEXT.md, then hand John a one-line `docs:` commit. (Full operating contract + the read-at-start/write-at-end loop: docs/COWORK-CONTEXT.md.)
...
Next.js 16 (App Router, TypeScript, Tailwind 4, Turbopack, no src/ directory). `app/(site)/cards/[slug]/page.tsx` is `export const dynamic = "force-dynamic"` (the R-008 mechanism — every curated render re-fetches the live eBay listing, never cached). [ADR-047](docs/DECISIONS.md#adr-047--ssgisr-hybrid-rendering--metadata-only-tier-for-the-18k-long-tail) tried an SSG+ISR hybrid to cache the cheap long tail but **reverted it** — the page's server-side `searchParams` read (variant/condition state) forces dynamic rendering and is incompatible with ISR (`DYNAMIC_SERVER_USAGE` 500s in prod). ISR is gated on first moving variant/condition selection client-side; see ADR-047's "Runtime reality" amendment + [R-013](docs/RISKS.md). What DID land from ADR-047: the **metadata-only tier** (cheapest render, no eBay/PokeTrace) + the **resumable bake**. See the "tiered per-card rendering" notes below.
...
The content engine generates a new blog post TWICE A WEEK (Mondays + Thursdays 14:03 UTC), runs 8 quality gates against the draft, and *can* publish directly to main with NO human review step (Vercel auto-deploys). The gates are the structural safety net — quality is enforced structurally, not editorially. **Current intended end state (John, 2026-07-02): approve-gated — `AUTO_PUBLISH_WEEKLY_POSTS=false`, drafts land in `app/blog/posts/_pending/` and ship through a human Approve/Skip step. The `blog-approval-loop` goal is the implementation (a Discord Approve/Skip + fact-grounding gates; one-click approve is the target end state, NOT a return to per-PR review). Leave the flag at `false` until that loop lands; do NOT flip it to `true` or "fix" it to ON without John.** ~~**SUPERSEDED (was, John 2026-06-25): "autonomy is intentionally ON — `AUTO_PUBLISH_WEEKLY_POSTS` true/unset + the Mon/Thu cron live; do not flip `=false` as a fix, autonomous publishing is intended."**~~ That auto-publish-to-main-unreviewed framing is retired as of 2026-07-02 (approve-gated is the intended state); the history is kept here so a future session doesn't re-derive the reversed rule. Mechanically: `AUTO_PUBLISH_WEEKLY_POSTS=false` routes drafts to `_pending/`; setting it `true` + uncommenting the Mon/Thu cron is what turns full autonomy back on.
...
Content-marker live verification is a standing closure-gate step alongside /security-review for any goal that touches blog/card content ([ADR-049](docs/DECISIONS.md#adr-049--content-pipeline-writeread-pinning--content-marker-verification-as-a-standing-closure-gate)). After deploy, run `CONTENT_VERIFY_BASE_URL=https://foiltcg.com npm test` (or curl-and-grep) so `lib/__tests__/content-marker-verification.test.ts` checks the RENDERED content (fabrications/dead-links absent, corrections present), not just HTTP 200. A 200 OK with wrong rendered content is worse than a 500 — nothing flags it (R-015 / PATTERNS I-006 + I-008).
When a goal touches content, verify the WRITER and the READER of any content directory resolve to the same path (the shared `lib/blog/posts-dir.ts` POSTS_DIR for blog posts) BEFORE claiming the edit reached production. Editing files the live route doesn't read is the R-015 trap.
When a goal modifies the content-engine `SYSTEM_PROMPT` (or any generation prompt), closure requires a **before/after regeneration measurement**, not just a passing test suite. Regenerate a post via the changed pipeline (route to `_pending` with `AUTO_PUBLISH_WEEKLY_POSTS=false` so nothing publishes) and report the concrete delta the change produced (e.g. net-new citations, attribution coverage, voiceCheck pass) in the SESSION-LOG — honestly, even if the lift is null ([ADR-050](docs/DECISIONS.md#adr-050--creator-content-ingestion--attribution-gate)). A prompt change whose output effect was never measured is not closed.
