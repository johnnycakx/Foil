...
Next.js 16 (App Router, TypeScript, Tailwind 4, Turbopack, no src/ directory). Per-card pages render via the SSG+ISR hybrid of [ADR-047](docs/DECISIONS.md#adr-047--ssgisr-hybrid-rendering--metadata-only-tier-for-the-18k-long-tail): `app/(site)/cards/[slug]/page.tsx` has `dynamicParams = true` + `revalidate = 3600` and an EMPTY `generateStaticParams` (nothing is prerendered at build → flat build time at any catalog size, zero build-time eBay calls). Curated cards stay dynamic-per-request via `await connection()` (the R-008 mechanism that replaced `force-dynamic` — eBay listing data is never cached); longtail + metadata-only cards are ISR-cached on-demand. See the "tiered per-card rendering" notes below.
