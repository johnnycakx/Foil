...
Foil's scanner reads name, set code, and collector number directly from the card image — including Japanese-language prints. Foil's scan data: across 25 Japanese cards processed in the last 30 days, the most common identification failure in manual lookups was using artwork description instead of collector number, which caused mismatches on secret rares where two cards share art but carry different numbers.
