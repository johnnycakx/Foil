...
Quick decoder: JP SAR = ENG SIR. JP SR (Super Rare) = ENG Ultra Rare (Full Art). JP UR (Ultra Rare) = ENG Hyper Rare (gold cards). The ladder shifts by one step at the top, which trips up a lot of buyers on eBay searches.
</Callout>
...
Japanese SARs are printed on slightly thinner card stock (~270 gsm vs the English ~300 gsm estimate) but receive a more aggressive foil embossing pass. The texture on a JP Venusaur ex SAR from Pokémon Card 151 (SV2a) or a later Scarlet & Violet era set catches raking light at a sharper angle. Under PSA's examination lamps, that texture reads as "intentional" and doesn't penalize the surface score — but it does make surface scratches harder to spot at raw inspection, which means more surprises after grading.
...
Don't conflate "harder to pull" with "lower PSA population." English SIRs are generally submitted for grading in higher volume than JP SARs of the equivalent card, simply because the English-speaking collector base is larger. Higher submission volume means faster-growing PSA pop counts on the English side, which can compress PSA 10 premiums faster — check the current PSA population report for the specific card rather than assuming.
</Callout>
...
<Callout variant="tip">
Before you decide which version to grade, get the exact card identity right — the set-code and collector-number combination is what determines the listing on PSA's population report, and confusing a 076/066 with an 077/066 is a $100+ mistake. Our [Pokémon card value calculator](/pokemon-card-value-calculator) pins the exact print before you price it.
</Callout>
...
If you're uncertain, Foil's image scan handles the three-field read automatically — including Japanese-language cards where the name is in katakana. See the [Pokémon card value calculator](/pokemon-card-value-calculator) to see how the pipeline turns those three fields into eBay sold, TCGplayer, and graded-ladder pricing.
...
_Updated 2026-05-30 with corrected valuations and card facts._
