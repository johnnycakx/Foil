...
## iter-28 - blackout-brand-and-deals-rework (bar hit iteration 2 of 5; John reviews live on the branch, NO PUSH)

**Skills invoked + the concrete change each drove:**
- **frontend-design** → the wordmark as a composed lockup, not a font swap: "Foil" keeps the bubble identity cut, "TCG" becomes a quiet second voice (0.5em caps, +0.18em tracking) so the gold reads as a hallmark stamp, not a second display face.
- **soft-skill** → the metallic execution grammar: a specular ramp (not flat fill), rest-position parked on the mid-gold so 12px caps read bright, hover sheen as a slow physical sweep (700ms strong ease-out), never an instant state change.
- **impeccable** → two catches: the /cards subhead shipped an em dash + the retired "watchlist" noun ("… eBay: best current deal, vault alerts, real prices" now); and the CONFLICT LOG the protocol requires: impeccable's absolute ban on gradient text is deliberately overridden here — John's verdict + Cowork's standing call specify bg-clip metal verbatim, and the locked brand decision beats the generic law (recorded in ADR-106 as a sanctioned exception).
- **emil-design-eng** → sheen choreography judged by frequency: the wordmark is seen hundreds of times/day, so NO ambient animation — the sweep exists only as a hover response, reduced-motion gated, and the rest state is static metal. Also the /deals dumbbell stays a drawn two-point delta (no fake easing curves).
- **redesign-skill lens (deals re-layout)** → parity-by-construction: ONE MoverRowItem for both directions instead of styling the heating list up to match (the audit-first rule: the defect was a component gap, imageUrl was always in the payload).

**Iter-1 (build):** wordmark lockup + .wordmark-tcg ramp (#856a00 anchor, #f4e3a1 specular, solid fallback, FoilTCG home accessible name); movers board rewritten to shared-row parity (designed null thumb, /cards links, 13px stats line, jump chips, "How to read this board" footnote); Workstream D migration of /start, /cards, /cards/sets to the night register (subagent execution on main-thread-distilled register rules; typeahead re-inked via the vault's sanctioned wrapper override, quiet header-band sakura only, no petals near inputs).
**Iter-2 (fixes from the shots):** gold rest-position 18%→38% (the anchor-heavy rest read muddy at 12px; mid-gold now sits on the glyphs, hover sweeps the specular through); /cards subhead em-dash + watchlist-noun fix; face trial shot side-by-side at nav scale → PICK: Bricolage bold caps for "TCG" (Shrikhand's italic bubble smears its counters at 12px and doubles the display voice; galleried both).

**Palette call logged (goal was teal-era):** the up-delta's "warm" color cannot exist inside current rules (vermillion = hanko-only, coral = hover-only, gold = wordmark-only), so heating draws its delta in luminous cream on the OPPOSITE side of the baseline; direction + label carry the semantics. Cooling keeps sakura (the buy signal).

Rubric: (a) matte charcoal, zero blue cast, cards the only glow 9.0 · (b) TCG metallic + only gold on night (guards pin it) 8.5 · (c) heating parity incl. images 9.0 · (d) /deals 5-second scannability (jump chips, big %, plain sentences) 8.6 · (e) register coherence home/deals/cards/start/card/w 8.8. avg 8.78 / min 8.5 — bar met at iteration 2. All three John verdicts visibly landed (charcoal ground pre-verified as shipped; wordmark + /deals this branch).
