...
## ADR-120 — Server-side funnel-event instrumentation (owned data, hashed visitor id, no analytics SaaS)

**Date:** 2026-07-14 · **Status:** Accepted · **Goal:** the `honest-comp-age` audit (Beat 3) · **Risk:** none new

**Context.** The north star is the first paying $6/mo subscriber, and the money path is proven working — the bottleneck is distribution, and John's plan is a ~$300 ad test. But there was NO server-side funnel instrumentation: `@vercel/analytics` gives raw page views (in Vercel's dashboard, unjoinable to our data), and `funnel-report.ts` could count trials but not see the middle. A blind ad test would return only the sale-or-no-sale bit, never WHERE a visitor dropped between landing and paying. Ratified by John this session: instrument before spend.

**Decision.** A single owned table, `funnel_events` (migration `20260714130000`), records five stages — `card_view → watch_set → pro_view → checkout_start → trial_start` — with the `utm_*` the `/pro` CTA already threads, joinable in `funnel-report.ts` against `subscriptions`. No third-party analytics SaaS. `lib/telemetry/funnel-events.ts` clones `logBrowseCall`'s discipline exactly (injectable client, never throws, fire-and-forget, **must not be awaited on the hot path**). PII posture matches `browse_calls` — the table stores operational metadata only: `visitor_id` is a one-way `SHA-256(salt | clientIp)` (unknown IP → `null`, never a fabricated id); no raw email, no raw IP. RLS enabled with no policies (service-role only). `card_view` fires for **attributed traffic only** (the page is `force-dynamic` + crawler-heavy; logging every render would swamp the table and teach nothing); the other four fire unconditionally. `checkout_start` fires the instant before the Stripe call, so it captures intent even when Stripe then fails (the exact preview-only failure ADR pinned by `pro-checkout-fallback.test.ts`).

**Consequences.** (1) The ad test becomes a diagnosis: `summarizeStageFunnel` prints ordered stage counts + step-to-step conversion, with holes reading as holes (count 0 / null denominator, never a fake ratio). (2) It degrades silently before the migration is applied — the report warns and shows "(no funnel events yet)", and every fire-and-forget write soft-fails, so the code is safe to deploy ahead of the table. (3) V1 counts EVENTS, not unique visitors (dedup by `visitor_id` is a later refinement if volume warrants). (4) `FUNNEL_VISITOR_SALT` is optional — a fixed build constant keeps the hash stable and non-reversible-in-practice; set the env var to rotate the visitor epoch or harden against an IP dictionary attack. (5) The migration is John's gate (a prod DB write); the code ships inert without it.

## ADR-119 — A sold figure carries its own date (asOfIso as a non-optional type) + sold_as_of ≠ computed_at

**Date:** 2026-07-14 · **Status:** Accepted · **Goal:** the `honest-comp-age` audit (Beats 1–2) · **Risk:** R-068, R-070

**Context.** The freshness gate (`isFreshStat`, `lib/cards/sold-coherence.ts`) admits any sold figure whose most recent sale is within `SOLD_FRESHNESS_MAX_DAYS` (35) — and then **threw the date away.** `SoldHeadlineModel` and `HeroSoldStat` carried no date field; only the DEGRADED paths (dated last-sale, dated listed fallback, honest "pending") were ever dated. So on the pass branch — measured at ~6 of 10 live card pages — a possibly-five-week-old number rendered as if current, under a hardcoded "Sold prices via PokeTrace · refreshed hourly" line that described our 1h SWR **poll**, not the **data**. Live prod (Moonbreon `swsh7-215`): "$2,285 · ~64 sales on record", undated, on ~13-day-old data. The freshness gate was never the lie; discarding the date was. This is R-068's silent-stale residual made concrete, and it sat inside R-070's blast radius (a card that stops trading keeps serving its last window).

**Decision.** A sold figure is a claim about a MOMENT, and the type system now enforces that it carries that moment. `SoldDisplay.windowed`, `SoldHeadlineModel`, and `HeroSoldStat` gain a **non-optional** `asOfIso`; `headlineFromRow` nulls an undated windowed row rather than emit it (belt to the freshness gate's braces). `lib/cards/comp-age.ts` is the one place a timestamp becomes copy — "as of Jul 14" at ≤1 day, "as of Jul 1 · 13 days ago" past that, so a reader never does date arithmetic to discover staleness. Every render surface threads it: card-page hero, sold-panel headline + per-tier table (tiers age independently), the `/start` binder's `soldLine` (the primary funnel), the seeded gift vault, and — the paid artifact — the alert email's `evidenceLine`. Where the alert's comp comes from the `market_movers` cache, a new column `sold_as_of` (migration `20260714120000`) carries the market's **last real trade**, kept strictly distinct from `computed_at` (when our cron cached the row): rendering `computed_at` as the comp's age would have moved the lie, since a nightly cron makes it always look ~1 day old. Nullable, no backfill — we never stored a past sale date and will not invent one; a null age is DISCLOSED ("sale date unknown"), never implied fresh. Beat 2 adds the missing gauge: `assessCompStaleness` (a market-pulse alarm `board-freshness` can't be, since that watches `computed_at`) + `scripts/staleness-scan.ts`, and a structural invariant test that fails if any headline ever renders undated again.

**Consequences.** (1) The honesty was in showing the age, not suppressing the figure — the first live scan (40 curated cards) found median rendered comp 8 days old, p90 18, 55% at 8+ days, none past 35d, so the alarm correctly stays quiet while every one of those now renders dated. (2) Enforced at compile time: every undated call site was a type error, which is how the fix reached all six surfaces without one being missed. (3) `sold_as_of ≠ computed_at` is the load-bearing distinction — the same class of bug (our-cache-time masquerading as the market's-time) is now structurally named. (4) The migration is John's gate; the code degrades to "sale date unknown" without it. (5) R-068's "sub-window silent staleness" residual is closed for the rendered surfaces; R-070's silent-stale failure mode is now visible rather than hidden.
