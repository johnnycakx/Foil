...
**Consequences.**
- **Coverage: 205/207 catalog cards baked** (Session 49 search matched 199; Session 49.1 added 6 via the overrides file). The remaining 2 are documented PokeTrace catalog gaps (LC Muk, Celebrations Mew #11) → graceful degradation. Flagships (Moonbreon, Rayquaza, Giratina, Lugia, Crown Zenith Charizards, Base Set Charizard incl. Shadowless, the SV-151 Charizard/Venusaur/Blastoise/Alakazam/Mew ex SIRs + Pikachu IR) all matched.
- **Override layer (Session 49.1).** `lib/cards/poketrace-overrides.json` (keyed by catalog slug) holds hand-resolved UUIDs for cards whose SDK number doesn't line up with PokeTrace's numbering — the SV-151 SIRs (SDK total 207 vs PokeTrace denom 165, and set name "151" is too short for the slug-suffix gate). The bake script consults overrides **before** the search heuristic; they win unconditionally. A `KNOWN_VENDOR_GAPS` map tags genuine vendor gaps in `docs/poketrace-bake-misses.md`.
