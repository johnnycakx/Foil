...
- **Measurement honesty.** The deterministic payload wins are **−64KB on the LCP-critical preloaded font + −27.6KB mobile JS** (~91KB off the mobile critical path), directly targeting the verified text-LCP. The authoritative real-prod AFTER LCP could NOT be certified headless this session: PSI's unkeyed quota is exhausted, the Vercel preview sits behind deployment protection (302), and localhost Lighthouse is pessimistic + noisy (the goal's own caveat — local before 3.4–4.3s, after 3.9s; overlapping). The <2.5s verdict needs a prod deploy + PageSpeed, matching how the prior perf sessions closed (build → John pushes → measures prod).
