...
**Status:** Approved (design). Amended 2026-06-06 with John's review verdicts — k=4, rotation slice 300/day, Set/Number corroborating, visitor-feed ~6h freshness threshold, §4 quota portfolio (bounded bot line), §9 two-tranche build plan. **Build-step-0 probe RUN 2026-06-06** ([findings](probe-findings-listing-aspects-2026-06-06.md)). **Tranche A COMPLETE in code 2026-06-11** — #1 core (certified 2026-06-07, [calibration](calibration-resolver-2026-06.md)), #2 per-card page, #3 /go redirect + wishlist cron, + the certified finish-aware query lever — committed locally, **NOT deployed; John reviews the I-009 measure first** (SESSION-LOG 2026-06-11). Tranche B (#4–#7) gated on the post-Tranche-A John checkpoint.
...
1. ✅ **`resolveVerifiedListing` core + fixtures — DONE 2026-06-07** (+ decision-accuracy certification: 0 false-accepts / 0 false-rejects at k=4). New `lib/listing/resolve.ts` (+ pure identity-gate module). Typhlosion fixtures + per-gate units pinned. Picker demoted to internal pre-filter.
2. ✅ **Per-card page migration — DONE 2026-06-11** (commit `33ad0c0`). Display + badge from one verdict (the badge reads the resolver's own getItem aspects — zero extra calls); `inferConditionLabel` deleted; honest-null state + search CTA; variants-panel marker only on the verified listing's matching finish variant (the §5 named defect's safe minimum); curated-null Product schema falls back to the baked TCGplayer AggregateOffer. Includes the **finish-aware query lever** (certified-safe; restricted to the exact WOTC `Rare Holo` rarity + explicit requested variants — modern rarities get no term until measured).
3. ✅ **`/go` redirect + wishlist cron migration — DONE 2026-06-11** (commits `a0ed8c5`, `7c78211`). The redirect 302s only to verified items (JP-fixture regression pinned end-to-end); the wishlist alerts only on verified condition matches (new `ANY_GRADED` condition + `lib/listing/condition-map.ts` token map + bgs-10-bl Black-Label title narrowing; browse-call accounting now counts real getItem spend against `MAX_BROWSE_CALLS`).
