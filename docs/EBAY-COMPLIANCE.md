...
**Status:** Living document. Last updated 2026-05-24 (Session 32). Maintenance protocol at the bottom.
...
| 4 | **Marketplace Account Deletion — public webhook required.** Production keysets that don't subscribe to deletion notifications (or apply for an exemption) are disabled. | Webhook route: [`app/api/webhooks/ebay-marketplace-deletion/route.ts`](../app/api/webhooks/ebay-marketplace-deletion/route.ts). Pure helpers (`challengeResponseHash`, `verifyNotificationSignature`, `handleChallenge`, `handleNotification`): [`lib/ebay-marketplace-deletion.ts:38`](../lib/ebay-marketplace-deletion.ts), [`:61`](../lib/ebay-marketplace-deletion.ts), [`:88`](../lib/ebay-marketplace-deletion.ts), [`:115`](../lib/ebay-marketplace-deletion.ts). Endpoint URL `https://foiltcg.com/api/webhooks/ebay-marketplace-deletion` registered in eBay developer portal 2026-05-24, keyset compliant. | [`lib/__tests__/ebay-marketplace-deletion.test.ts`](../lib/__tests__/ebay-marketplace-deletion.test.ts) — 18 tests pinning challenge-hash fixture, HMAC verify, GET/POST status paths, synchronous-completion invariant. |
| 5 | **Marketplace Account Deletion — handler must respond within 3 seconds, must not persist user data.** | Synchronous handler — no DB writes, no outbound fetches, no awaited work beyond `request.text()`. POST returns `{ acknowledged: true }` and discards the payload. See `route.ts` + the explicit "ack and discard" comment in `handleNotification` ([`lib/ebay-marketplace-deletion.ts:107-129`](../lib/ebay-marketplace-deletion.ts)). | Same test file — "handleChallenge + handleNotification complete synchronously — no awaited externals" pins that the decision functions don't return Promises. |
...
- [ ] `grep -r "api.ebay.com" lib/ app/ --include='*.ts' --include='*.tsx' | grep -v __tests__` returns only `lib/affiliate/ebay-browse.ts` + `lib/affiliate/ebay-oauth.ts`
