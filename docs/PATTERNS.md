...
**Promotion trigger.** This is now the third+ instance. Promote the "single-path-constant + scan-every-surface-for-literals" recipe to a dedicated ADR if a fourth surface drifts (e.g. a Vercel config, a Dockerfile, a cron script outside `scripts/`).
