...
**Mitigation — Path B ACCEPTED (residential scheduled box, pilot).** Ingestion runs on John's local Windows machine via Task Scheduler job `FoilTranscriptIngest` (daily 06:00), invoking `scripts/ingest-and-push.ps1`: `--cookies-from-browser chrome` (live session, **auto-refresh, no secret, no rotation cadence**) → digest → push `docs/transcript-digests/` to `main`. Setup + failure modes in [docs/runbooks/local-ingest-cron.md](runbooks/local-ingest-cron.md). **The Mon/Thu content engine reads the digest from `main`, so it doesn't matter that the box, not CI, produced it.** Note: graduate to a **residential proxy** (add `--proxy` + a proxy secret to the dormant CI workflow) if local-box uptime becomes a problem. The `YT_DLP_COOKIES` GH secret + the CI workflow are retained (dormant) for that future proxy path.
