...
**Severity:** Medium
**Status:** `mitigating` (added + largely closed 2026-06-29, [ADR-083](DECISIONS.md#adr-083--resend-is-the-sole-send--unsubscribe-surface-beehiiv-is-passive-fixes-the-silent-beehiiv-unsubscribe-no-op)). The forward leg is fixed + live-verified; one dashboard residual remains. **2026-07-01 ([ADR-090](DECISIONS.md#adr-090--start-funnel-integrity-tri-store-opt-in-idempotent-watches-attribution-and-an-unsubscribe-that-stops-alerts)): the WORST CAN-SPAM leg — the wishlist-alert one-click unsubscribe that stopped nothing (Beehiiv-only; the alert cron kept sending forever; the page said "email john… to stop") — is closed.** `/api/unsubscribe` now stops the newsletter (Supabase + Beehiiv via `syncUnsubscribe`) AND pauses every `watchlists` row (`alerts_paused_at`, cron-excluded); spam complaints via the Resend webhook pause alerts too. Remaining residuals: the Beehiiv "Foil welcome" automation pause (dashboard-only, below) and double-opt-in (deferred, IDEAS). **2026-07-11 (funnel-stress-test): a THIRD leg found + closed — the one-click path left the RESEND audience contact `unsubscribed:false`** (`syncUnsubscribe` was written for the Resend-webhook direction, where Resend had already flipped the flag, and reused verbatim for the HMAC one-click direction) — since the weekly digest is a Resend Broadcast to that audience, an opted-out address would have received the next marketing send. Fixed with a third `resendUnsubscribe` leg (live-verified flag flip + test-pinned). Note for the playbook below: the funnel-stress walk showed the "Foil welcome" automation is currently the ONLY post-signup email a deals_gate subscriber receives — pausing it (playbook step 1) removes the funnel's sole welcome touch, so pair any pause with the daily-drop/welcome follow-on, don't just switch it off.
...
**2026-07-05 amendment — the deals-freshness diagnosis materially weakens the "keep paying" case (docs/goals/_results/deals-freshness-diagnosis.md).** Measured: the movers board reads PokeTrace's **eBay windowed SOLD averages** (source order `[ebay,tcgplayer,cardmarket]`; all 22 probed cards resolved eBay-NM), and those aggregates **refresh only every ~2–3 days and go weeks-stale on thin cards** — i.e., PokeTrace's *unique* contribution to the spine (eBay windowed sold) is precisely the stale, board-freezing signal. The fresher alternative (eBay **lowest-active**) is one **we already own via the eBay Browse client** (card-page/`/go`). Live sold ground-truth also showed PokeTrace/TCGplayer market can be inflated ~2× on vintage (Base Blastoise $229 vs true raw ~$95). Net for the ~Aug 10 benchmark: if eBay-lowest-active (owned, $0) can carry the movers/deal-signal, the case for $98/mo PokeTrace narrows to *graded/pop + card-page windowed sold*, not the deal engine. Fold this into the benchmark before the next renewal.
...
- **Residual snapshot-staleness surface CLOSED (2026-07-08, content-trust-hotfix Defect 1, [ADR-110](DECISIONS.md#adr-110--extending-the-adr-104-freshness-doctrine-to-the-baked-lines-snapshot--tcgplayer-listed-labeling)).** The `/lines` baked path had bypassed the ADR-104 freshness doctrine entirely: `scripts/seed-line-sold.ts` selected `avg30d ?? avg` (the `?? avg` = an all-time last sale, exactly the anti-pattern ADR-104 forbids), and the render made no freshness claim-check. Now the selection is FRESH-WINDOWED ONLY (`lib/lines/sold-select.ts::resolveLineSoldEntry` reuses `freshWindowedValue`/`isFreshStat`), each entry records `soldAsOf`, and `soldPhrase` degrades a figure past the 35-day window to dated "last sold (as of <date>)". Regenerating the snapshot corrected 8 figures the old all-time fallback had skewed (e.g. `neo2-32-umbreon $175→$72`, `neo2-20-espeon $93→$28` — ~2× inflations) and honestly suppressed 1 stale entry. NOTE: the specific brief premise "neo2-13 Umbreon $840 is ~2× inflated vs ~$450 live" was CONTRADICTED by a live PokeTrace probe — $840 is the current fresh 30-day windowed NM avg (med $750, n=20, updated 2026-06-07); the "$450" was a different basis (lowest active ask), so $840 was left intact (sourced-correct). A scheduled snapshot refresh is still the follow-up if lines multiply.
...
**Status:** `resolved` (by design, 2026-07-02 — eve-vault-template-claims, [ADR-100 amendment](DECISIONS.md#adr-100--seeded-gift-vaults-pre-made-claimable-vaults-on-a-second-token-context-eve-vault)). John's pre-push correction reframed the object model: a seeded vault is a TEMPLATE — every email that submits gets its own independent watch-set instance, unlimited, and the public page never locks. "Squatting" is definitionally impossible because there is nothing exclusive to squat: a stranger claiming ahead of eve just becomes a converted subscriber, and eve gets the identical experience whenever she clicks. The composite-PK instantiation log (`(vault_slug, claimed_email)`) keeps per-email idempotency; abuse is bounded by the per-IP rate limit, honeypot, and per-email watch-cap. The original entry below is kept for history.

**Original entry (superseded):** `monitoring`. Inherent to the eve-vault design ([ADR-100](DECISIONS.md#adr-100--seeded-gift-vaults-pre-made-claimable-vaults-on-a-second-token-context-eve-vault)) — "the first email submitted CLAIMS the vault" is the activation mechanic, and the link ships in a public reply thread. A squatter gains nothing sensitive (the page shows only their own masked email; alerts go to their inbox; the claimant's personal vault link travels by email only), but the dedication would be watching the wrong inbox.
...
**2026-07-08 amendment — three residual live surfaces closed (content-trust-hotfix, [ADR-110](DECISIONS.md#adr-110--extending-the-adr-104-freshness-doctrine-to-the-baked-lines-snapshot--tcgplayer-listed-labeling)).** (1) The `/lines` baked snapshot bypassed the ADR-104 gate (all-time `?? avg` fallback, no render freshness-check) — now fresh-windowed-only + dated-last-sale degradation (see R-066). (2) The **TCGplayer `market`** figure on the variants panel + `/lines` was rendered as a bare "market"/"to buy right now" price though it runs ~2× real sold on low-liquidity vintage (Base Blastoise $229 listed vs ~$95 sold) — now labeled EXPLICITLY "TCGplayer listed" with a may-lag caveat (the coherence gate is PokeTrace-sold-only and does not reach TCGplayer market, so the honest label is the fix, not suppression — for high-liquidity moderns like Moonbreon the listed $2,276 ≈ sold $2,285, so blanket suppression would wrongly hide an accurate figure). (3) **Cross-post figure contradiction** resolved: two live posts undervalued Moonbreon (Umbreon VMAX Alt Art, EVS 215) at ~$180 NM / $175-220 PSA9 / $310-380 PSA10, ~12× below the sourced live PokeTrace windowed sold (eBay NM $2,285 / PSA9 $2,277 / PSA10 $4,574) — reconciled to the sourced numbers, which matched the (already-correct) checklist post; pinned with content markers.

**2026-07-14 amendment — the sub-window silent-staleness residual is closed on the rendered surfaces (honest-comp-age, [ADR-119](DECISIONS.md#adr-119--a-sold-figure-carries-its-own-date-asofiso-as-a-non-optional-type--sold_as_of--computed_at)).** Residual (2) above understated the problem: the 35-day gate didn't only make a 36-day figure degrade — INSIDE the window it rendered a 1-day-old comp and a 34-day-old comp IDENTICALLY, undated. Measured ~6 of 10 live card pages carried an undated "30-day sold avg," all under a hardcoded "refreshed hourly" caption that described our poll, not the data (Moonbreon: $2,285, undated, on 13-day-old data). Fixed: a sold figure now carries its date as a NON-OPTIONAL type (`asOfIso`), rendered on every surface (hero, sold panel + per-tier table, /start binder, gift vault, and the alert email's evidence line) via `lib/cards/comp-age.ts`; an undated windowed figure nulls out rather than renders. The alert's comp gained `market_movers.sold_as_of` (the market's last trade, distinct from `computed_at`, our cache time). A new gauge (`assessCompStaleness` + `scripts/staleness-scan.ts`) measures the age distribution `board-freshness` couldn't see, and an invariant test fails the build if any headline renders undated again. First scan (40 curated): median 8d, p90 18d, none >35d — the alarm stays quiet; the honesty was in showing the age. **Migration `20260714120000` is John's gate; the code degrades to "sale date unknown" without it.**

**2026-07-05 amendment — the /deals movers board is a fresh surface of this risk (docs/goals/_results/deals-freshness-diagnosis.md).** The freshness-diagnosis walk found residual (3) manifesting concretely on the good-buys board: on heterogeneous **raw vintage**, eBay-sourced NM and TCGplayer-sourced NM diverge **2×+**, and live eBay sold ground-truth shows the true raw price is a **wide distribution** ($65–$135 Base Blastoise; $12–$45 Ninetales) where any single "NM" point-estimate is a fiction — so the board renders a **false precise-% "good buy"** ("Base Blastoise −12.3%") that is actually inside the noise band. Note eBay was the *more* accurate source there (TCGplayer $229 inflated), so "prefer the deeper source" is the wrong reflex. Mitigation to spec with the deals-basis follow-on: gate heterogeneous raw vintage out of the precise-% good-buys surface (or widen to a confidence range), and extend the cross-source coherence gate to the movers path.
...
**Severity:** High
**Status:** `mitigating` — **the lapse is now SURVIVABLE** (pricing-bridge 2026-07-14, [ADR-118](DECISIONS.md#adr-118--price-basis-is-a-type-not-a-caption--the-listed-fallback-that-survives-a-dead-sold-vendor)): a dead/lapsed key degrades to a labeled, dated TCGplayer LISTED price on **82.3% of the catalog (2,673/3,248 cards)** instead of the site-wide "Sold data pending" blank it produced before. Renewal decision is still John's, due ~Jul 15.

> **⚠️ EVIDENCE UPDATE 2026-07-14 (re-probed live, contradicts the input to the "don't renew" call).** The spike's headline finding — the eBay sold ingest **frozen at 2026-07-05**, "not one tier newer" across ~35 cards — **no longer holds. The ingest has RESUMED.** Re-probed today: the key still authenticates (HTTP 200), and high-velocity cards carry eBay sold reads **1–3 days old** (Blastoise Base `2026-07-13`, Umbreon VMAX ES `2026-07-12`, Pikachu ex SS `2026-07-13`, Gengar EX `2026-07-13`). **But freshness is now wildly UNEVEN rather than uniformly frozen, which is the more dangerous failure mode:** Moonbreon `swsh7-215` — the flagship $2,285 control — is still **13 days stale** (`2026-07-01`), and low-velocity cards run 28–102+ days. Before ADR-118 the page rendered a 1-day-old read and a 102-day-old read *identically*, as if both were current. That is the silent-stale lie the freshness gate + age label now prevent. **Net: the "they won't fix it / don't renew" premise was formed on now-stale evidence and deserves a re-look before the renewal — but the vendor-risk case (anonymous operator, 1-year domain, no uptime history, uneven ingest) is UNCHANGED, and ADR-118 is what makes either decision safe.**
...
**Mitigation.** (1) ✅ **SHIPPED (ADR-118):** price provenance is a TYPE (`{source, basis: sold|listed|guide, lastUpdated}`), a sold-labeled surface can never receive a non-sold-basis figure (compile-time + runtime + pinned test), and a dark sold spine degrades to a labeled/dated LISTED price rather than a blank. (2) ✅ **SHIPPED, and it needed NO new vendor:** the listed fallback reads the `tcgplayerPrices` **already in the committed catalog snapshot** — so tcgcsv was not needed (zero new keys, zero id-mapping, zero network, and it cannot fail with PokeTrace because it never touches it). A weekly `--refresh-prices` cron keeps those prices inside the 30-day window (they were frozen at first-bake: median 13 days, worst 1,231 — a fallback that rots is not a fallback). (3) ✅ **SHIPPED (ADR-119, 2026-07-14):** the silent-stale failure this risk warns about is now VISIBLE, not hidden — every sold figure carries its date, and a catalog-wide staleness gauge (`assessCompStaleness` + `scripts/staleness-scan.ts --alarm`) pings #errors on a systemic cooling (>50% of a ≥20 sample past 35 days), which `board-freshness` (watching only `computed_at`) structurally could not detect. This gives the renewal decision an evidence source: run the scan to SEE how uneven the spine is, rather than infer it. (4) Trial keys for Scrydex Growth + JustTCG — still open. (5) Apply for eBay Marketplace Insights (own the sold source) — **still open, still the permanent fix; John's step.**
