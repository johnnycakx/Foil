...
**Severity:** Medium
**Status:** `mitigating` (added + largely closed 2026-06-29, [ADR-083](DECISIONS.md#adr-083--resend-is-the-sole-send--unsubscribe-surface-beehiiv-is-passive-fixes-the-silent-beehiiv-unsubscribe-no-op)). The forward leg is fixed + live-verified; one dashboard residual remains. **2026-07-01 ([ADR-090](DECISIONS.md#adr-090--start-funnel-integrity-tri-store-opt-in-idempotent-watches-attribution-and-an-unsubscribe-that-stops-alerts)): the WORST CAN-SPAM leg — the wishlist-alert one-click unsubscribe that stopped nothing (Beehiiv-only; the alert cron kept sending forever; the page said "email john… to stop") — is closed.** `/api/unsubscribe` now stops the newsletter (Supabase + Beehiiv via `syncUnsubscribe`) AND pauses every `watchlists` row (`alerts_paused_at`, cron-excluded); spam complaints via the Resend webhook pause alerts too. Remaining residuals: the Beehiiv "Foil welcome" automation pause (dashboard-only, below) and double-opt-in (deferred, IDEAS).
