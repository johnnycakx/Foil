...
| 8 | **eBay developer account appeal — resubmit for Browse API access** <!-- added via cowork 2026-05-23 --> | First application was auto-rejected. EPN access is sufficient for V1, but Browse API unlocks richer filtering (seller-feedback ≥98%, condition NM/LP/MP) needed for the "best deal" surface to actually be best. Appeal text drafted in Cowork session 2026-05-23 with explicit compliance posture (no AI training on eBay data, no resale, render-time fetches only) addressing the 2025 License Agreement update. eBay's appeal site was down at time of capture — retry when service restored. | John (manual via developer.ebay.com → Help with My Developer Account → "My account registration was rejected") | Pending — eBay site down 2026-05-23 |
