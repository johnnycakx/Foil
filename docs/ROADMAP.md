...
| 8 | **eBay developer account appeal — resubmit for Browse API access** <!-- escalated via Session 21 EPN investigation 2026-05-23 --> | **LOAD-BEARING for V1 best-listing surface.** First application was auto-rejected. EPN credentials (Account SID + Auth Token) authenticate ONLY the affiliate Transaction Detail Report API + URL-wrapping; they do NOT authenticate any product-search endpoint. Finding API was decommissioned 2025-02-05. Browse API is the only real-time listing-search path and requires this appeal. Without it, `lib/affiliate/epn.ts::getBestListing` returns `null` on every call and per-card pages render the fallback "Browse on eBay" CTA (affiliate-tracked but no curated best listing). [ADR-021 amendment](DECISIONS.md#adr-021--epn-as-v1-live-listing-source-browse-api-deferred) documents the surface-area discovery. Appeal text drafted in Cowork session 2026-05-23 with explicit compliance posture (no AI training on eBay data, no resale, render-time fetches only) addressing the 2025 License Agreement update. | John (manual via developer.ebay.com → Help with My Developer Account → "My account registration was rejected") — **try every 24h until accepted.** | Pending — escalated to load-bearing 2026-05-23 |
