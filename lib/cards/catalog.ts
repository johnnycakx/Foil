...
// Breadth expansion (top-5-per-set goal, extends ADR-046). Generated by
// scripts/expand-top5-per-set.ts: every English set's top-5-by-value cards not
// already present above. Where the wave-1 LONGTAIL_CATALOG added DEPTH across
// the 29 catalog sets, this adds BREADTH — the chase cards of the ~144 sets that
// were missing entirely (the content-engine root cause, John 2026-06-30). Kept
// in its own generated file so regeneration is a clean overwrite and the
// ≤5-per-set invariant is testable in isolation (lib/__tests__/catalog.test.ts).
// tier "longtail" → renders SDK price (AggregateOffer + variants) + affiliate
// CTA; PokeTrace sold-history stays LAZY (no bulk baking) and resolves
// on-demand when a card is surfaced.
